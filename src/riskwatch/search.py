...
def _clean(url):
    try:
        p=urlparse(url)
        if p.netloc.endswith("duckduckgo.com") and p.path.startswith("/l/"): return unquote(parse_qs(p.query).get("uddg",[url])[0])
...
def search(query, limit=None):
    ...
    for source,url in endpoints:
        try:
            ...
            for node in soup.select(".result" if source=="DuckDuckGo" else "li.b_algo")[:limit]:
                ...
                out.append({"source":source,"url":_clean(a.get("href","")),"title":a.get_text(" ",strip=True),"snippet":sn.get_text(" ",strip=True) if sn else "","query":query})
            if out: return out
