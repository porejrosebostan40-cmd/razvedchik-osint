...
from .forecast import build_forecast, forecast_record, resolve_forecasts, calibration_summary, calibrate_probability, render_context
...
def _throttled_decision(store):
    ...
    if previous:return {k:previous.get(k,v) for k,v in {'probability':0,'confidence':0,'risk':0,'decision':'WATCH','reason':'AI call throttled; previous decision reused','signals':[],'missing_indicators':[],'next_event':'UNKNOWN','horizon':'UNKNOWN','forecast_basis':'','pattern':{},'scenario_answer':'UNKNOWN','analysis_provider':'cached'}.items()}
    return None
...
def _episode_duplicate(store,forecast,evidence_ids):
    ...
    fp=forecast_record(forecast,0,evidence_ids=evidence_ids).get('evidence_fingerprint'); now=datetime.now(timezone.utc).timestamp()
...
def run():
    ...
    cal=calibrate_probability(model_probability,resolved)
    calibrated_probability=float(cal.get('probability',0))
    decision['model_probability']=model_probability;decision['probability']=calibrated_probability;decision['probability_calibration']=cal
    ...
    if not _episode_duplicate(store,pattern,evidence_ids):store.save_forecast(forecast_record(pattern,model_probability,evidence_ids=evidence_ids))
    store.save_decision(decision)
    p=float(decision.get('probability',0));risk=int(decision.get('risk',0));c=int(decision.get('confidence',0));answer=decision.get('scenario_answer','UNKNOWN'); calibrated=cal.get('status') in ('preliminary','measured')
