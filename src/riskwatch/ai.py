...
def _compact_events(events, limit=8):
    selected=[]; seen=set()
    for e in reversed(events):
        kind=str(e.get("kind",""))
        if kind not in seen:
            selected.append(e); seen.add(kind)
...
def analyze(events):
    ...
    payload={
        ...
        "instructions":SYSTEM,
        "input":("Return only valid JSON. " + json.dumps({"scenario":"мобилизационные или связанные с ФСИН действия после 20 сентября 2026 года","events":_compact_events(events)},ensure_ascii=False)),
    ...
    try:
        ...
        for k in ('probability','confidence','risk'): result[k]=_score(result.get(k,0))
        result.setdefault("decision","WATCH"); result.setdefault("reason","OpenAI analysis completed"); result.setdefault("signals",[]); result.setdefault("missing_indicators",[]); result["analysis_provider"]="openai"
        return result
